...
Defaults:
  ...
  share root     ~/shared
  remote command cd "$HOME/shared/UNIX V6++V1/oos/targets/UNIXV6++" && export BXSHARE=/usr/share/bochs && bochs-gdb -q -f bochsrc.bxrc
...
remote_command="${OOS_HOST_COMMAND:-cd \"${remote_repo}/targets/UNIXV6++\" && export BXSHARE=\"${remote_bxshare}\" && exec bochs-gdb -q -f bochsrc.bxrc}"
interactive_shell=0
