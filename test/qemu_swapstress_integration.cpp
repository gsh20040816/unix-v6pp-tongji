...
#ifndef OOS_QEMU_SWAPSTRESS_IT_ROOT_DIR
#define OOS_QEMU_SWAPSTRESS_IT_ROOT_DIR "."
#endif

#ifndef OOS_QEMU_SWAPSTRESS_IT_BUILD_DIR
#define OOS_QEMU_SWAPSTRESS_IT_BUILD_DIR "."
#endif
...
{
	const Config config = {
		ReadEnvInt("QEMU_SWAPSTRESS_PROMPT_TIMEOUT_SEC", 90),
		ReadEnvInt("QEMU_SWAPSTRESS_CMD_TIMEOUT_SEC", 300),
		ReadEnvInt("QEMU_SWAPSTRESS_SHUTDOWN_TIMEOUT_SEC", 45),
		ReadEnvString("QEMU_BIN", "qemu-system-i386"),
		ReadEnvString("QEMU_SWAPSTRESS_READY_MARKER", "OOS_BOOT_SHELL_READY")
	};

	std::filesystem::path rootDir =
		ReadEnvString("OOS_ROOT_DIR", OOS_QEMU_SWAPSTRESS_IT_ROOT_DIR);
	std::filesystem::path runScript = rootDir / "qemu" / "run_qemu.sh";
	std::filesystem::path buildDir =
		ReadEnvString("OOS_BUILD_DIR", OOS_QEMU_SWAPSTRESS_IT_BUILD_DIR);
	std::filesystem::path imagePath = buildDir / "c.img";
	std::filesystem::path stdoutLogPath = buildDir / "qemu-swapstress-stdout.log";
	std::filesystem::path stderrLogPath = buildDir / "qemu-swapstress-stderr.log";
	...
	std::cout << "qemu swapstress integration: prompt_timeout="
		      << config.promptTimeoutSec << "s cmd_timeout=" << config.cmdTimeoutSec
	...
	if ( ok )
	{
		if ( !SendCommand(child.stdinFd, "swapstress") )
		{
			std::cerr << "failed to send swapstress command\n";
			ok = false;
		}
	}

	ok = ok && WaitForCondition(
		"swapstress completion",
	...
	if ( ok && ContainsFrom(stdoutBuffer, stdoutCursor, "swapstress: FAIL") )
	{
		std::cerr << "swapstress reported failure\n";
		ok = false;
	}
	if ( ok && ContainsFrom(stderrBuffer, 0, "Panic") )
	{
		std::cerr << "kernel reported Panic during swapstress\n";
		ok = false;
	}
	...
	if ( !ok )
	{
		PrintTail("qemu swapstress stdout", stdoutBuffer);
		PrintTail("qemu swapstress stderr", stderrBuffer);
		return 1;
	}
	...
	std::cout << "qemu swapstress integration passed: qemu exit code="
		      << qemuExitCode << "\n";
	return 0;
}
